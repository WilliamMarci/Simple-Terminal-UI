...
class CanvasRender:
	...
	def summonRectange(width,height,leftline=BOX[1],rightline=BOX[1],topline=BOX[0],bottomline=BOX[0],corner_LU=BOX[2],corner_RU=BOX[3],corner_LD=BOX[4],corner_RD=BOX[5]):
		content=[]
		#first line
		firstline=[]
		firstline.append(corner_LU)
		for i in range(width-2):
			firstline.append(topline)
		firstline.append(corner_RU)
	...
	def recToCanvas(x,y,rec_content,canvas_content,ifCover=False):
...
class Scatter(UIElement):
	...
	def drawCanvas(self):
		...
		#summun edge
		edge=CanvasRender.summonRectange(canvas_width,canvas_height,BOLDBOX[1],BOLDBOX[1],BOLDBOX[0],BOLDBOX[0])
		self.content=CanvasRender.recToCanvas(canvas_x,canvas_y,edge,self.content,True)
